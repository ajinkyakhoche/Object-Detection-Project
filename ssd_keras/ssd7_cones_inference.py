...
model_path = 'ssd7_epoch-20_loss-1.9493_val_loss-1.0999.h5'
...
files.sort()
image_paths = [image_path+str(item).zfill(6)+".jpg" for item in files]
print(image_paths)

orig_images = [plt.imread(image_path) for image_path in image_paths]
...
for i in range(0, 10):#len(orig_images)):
...
dataset.parse_xml(images_dirs=[test_images_path],
                  image_set_filenames=[test_setFileName],
                  annotations_dirs=[test_annotations_path],
                  classes=classes,
                  include_classes='all',
